...
export const SkillModal: React.FC<SkillModalProps> = ({ skill }) => {
  if (!skill) {
    return (
      <div className="flex items-center justify-center h-full p-8 bg-white rounded-lg shadow-sm">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-brand-primary">Welcome to the ADM Career GPS Wiki</h2>
          <p className="mt-2 text-text-secondary">Please select a skill from the sidebar to view its details.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <article className="bg-white rounded-lg shadow-sm p-6 md:p-8">
      ...
      <section className="space-y-6">
          <Accordion title="What It Looks Like (Exemplars)" defaultOpen={true}>
          ...
          <Accordion title="Proof Points (Evidence of Mastery)" defaultOpen={true}>
          ...
          <Accordion title="Real Story Example" defaultOpen={true}>
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                  <p className="font-semibold text-text-primary">{skill.realStoryExample.persona}</p>
                  <blockquote className="mt-2 pl-4 border-l-4 border-brand-secondary/50 space-y-2 text-text-secondary">
                      <p><strong className="text-slate-600">Challenge:</strong> {skill.realStoryExample.challenge}</p>
                      <p><strong className="text-slate-600">Action:</strong> {skill.realStoryExample.action}</p>
                      <p><strong className="text-slate-600">Result:</strong> {skill.realStoryExample.result}</p>
                  </blockquote>
                  <div className="mt-4">
                      <strong className="text-slate-600">Skills Used:</strong>
          ...
          <Accordion title="Common Mistakes to Avoid" defaultOpen={true}>
              <ul className="list-disc pl-5 space-y-2 text-text-secondary">
          ...
          <Accordion title="Interview Questions" defaultOpen={true}>
              <ul className="list-disc pl-5 space-y-2 text-text-secondary">
